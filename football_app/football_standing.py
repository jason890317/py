...
def check_update(leauge):
    ...
    with open(leauge+"_update_check", "r") as file:
    ...
    if difference >= timedelta(days=1):
        ...
        with open(leauge+"_update_check","w") as file:
...
def playball(leauge, picture_gen=False):
    ...
    if check_update(leauge):
        logging.log(logging.INFO,leauge+" data is being updated..")
        ...
        with open("./"+leauge+".data.json","w") as json_file:
            json.dump(standing,json_file,indent=4)
    else:
        logging.log(logging.INFO,leauge+" Data has been updated today")
        with open("./"+leauge+".data.json","r") as json_file:
            standing = json.load(json_file)
    return standing
